...
#    model_face.summary()
    ...
    def FeatureExtract():
        ...
        x = model_face.get_layer("conv2d_8").output
        x = GlobalAveragePooling2D()(x)
        encoder = Model(input=model_face.input, output=x)
        return encoder
#                
#    sess = tf.Session()
#    featureVec=GlobalAveragePooling2D()(model_face.get_layer("conv2d_8").output)
#    with sess.as_default():
#        
#        featureVec = featureVec.eval()
#    print("features",featureVec)
#    print("type",type(featureVec))
    ...
    for i in range(len(results)):
        ...
        face_image = img_cp[ymin:ymax, xmin:xmax]
        croppedFaces.append(face_image)
        face_image = cv2.resize(face_image, (416,416))
        face_image = np.expand_dims(face_image, axis=0)
        FeatureModel = FeatureExtract()
        croppedFaceFeatures.append(FeatureModel.predict(face_image)[0])
#        out2 = model_face.predict(img_camera)[0]
        
    return croppedFaces,img_cp,croppedFaceFeatures 
